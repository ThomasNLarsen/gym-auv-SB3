...
class Viewer2D(object):
    def __init__(self, width : int = VIDEO_W, height : int = VIDEO_H, display=None):
...
def _render_indicators(env, W, H):
    ...
    env._viewer2d.lambda_value_field.text = "{:2.2f} (max. {:2.2f})".format(env.rewarder._vessel.speed,
                                                                            env.rewarder._vessel.max_speed)
    env._viewer2d.lambda_value_field.draw()

    env._viewer2d.eta_text_field.text = "CTE:"
    env._viewer2d.eta_text_field.draw()
    env._viewer2d.eta_value_field.text = "{:2.2f}".format(
        env.rewarder._vessel.req_latest_data()['navigation']['cross_track_error'])
    env._viewer2d.eta_value_field.draw()

    env._viewer2d.input_text_field.text = "Input:"
    env._viewer2d.input_text_field.draw()
    env._viewer2d.input_value_field.text = "Thrust: {:2.2f} [N], Moment: {:2.2f} [Nm]".format(env.vessel._input[0],
                                                                                              env.vessel._input[1])
...
def init_env_viewer(env):
    env._viewer2d = Viewer2D(int(VIDEO_W), int(VIDEO_H))
    ...
    env._viewer2d.cum_reward_text_field = pyglet.text.Label('0000', font_size=10,
                                                            x=20, y=WINDOW_H - 50.00, anchor_x='left',
                                                            anchor_y='center',
                                                            color=(0, 0, 0, 255))
    env._viewer2d.cum_reward_value_field = pyglet.text.Label('0000', font_size=10,
                                                             x=260, y=WINDOW_H - 50.00, anchor_x='right',
                                                             anchor_y='center',
                                                             color=(0, 0, 0, 255))
    ...
    env._viewer2d.time_step_value_field = pyglet.text.Label('0000', font_size=10,
                                                            x=260, y=WINDOW_H - 70.00, anchor_x='right',
                                                            anchor_y='center',
                                                            color=(0, 0, 0, 255))
    ...
    env._viewer2d.episode_value_field = pyglet.text.Label('0000', font_size=10,
                                                          x=260, y=WINDOW_H - 90.00, anchor_x='right',
                                                          anchor_y='center',
                                                          color=(0, 0, 0, 255))
    ...
    env._viewer2d.lambda_value_field = pyglet.text.Label('0000', font_size=10,
                                                         x=260, y=WINDOW_H - 110.00, anchor_x='right',
                                                         anchor_y='center',
                                                         color=(0, 0, 0, 255))
